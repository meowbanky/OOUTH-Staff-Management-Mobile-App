...
function getCategories($coop, $type) {
    $stmt = $coop->prepare("SELECT id, name FROM coop_categories WHERE type=? ORDER BY name");
...
$incomeCats = getCategories($coop, 'income');
$expendCats = getCategories($coop, 'expenditure');
?>
